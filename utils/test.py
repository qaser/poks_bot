...
def migration_emergency_stops():
    queryset = list(emergency_stops.find({}))
    for es in queryset:
        agr = gpa.find_one({'ks': es['station'], 'num_gpa': es['gpa']})
        try:
            emergency_stops.update_one(
                {'station': es['station'], 'gpa': es['gpa']},
                {'$set': {'gpa_id': agr['_id']}}
            )
        except:
        ...
        try:
            ...
            if check is not None:
                groups.update_one(
                    ...
                    {'$set': {'gpa_id': agr['_id'], 'ao_id': es['_id']}}
                )
        except:
            gpa_name = ''
